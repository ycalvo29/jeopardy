...
function validateNoAuthenticationOnInterfaces(metadata, errorCollector) {
    ...
    [authenticatedDirective, requiresScopesDirective, policyDirective].forEach((directive) => {
        for (const application of directive.applications()) {
            ...
            function isAppliedOnInterface(type) {
                return (0, definitions_1.isInterfaceType)(type) || isInterfaceObjectType((0, definitions_1.baseType)(type));
            }
            function isAppliedOnInterfaceField(elem) {
                return (0, definitions_1.isFieldDefinition)(elem) && isAppliedOnInterface(elem.parent);
            }
            if (isAppliedOnInterface(element) || isAppliedOnInterfaceField(element)) {
                ...
                errorCollector.push(error_1.ERRORS.AUTHENTICATION_APPLIED_ON_INTERFACE.err(`Invalid use of @${directive.name} on ${kind} "${element.coordinate}": @${directive.name} cannot be applied on interfaces, interface objects or their fields`, { nodes: (0, definitions_1.sourceASTs)(application, element.parent) }));
            }
        }
    });
}
